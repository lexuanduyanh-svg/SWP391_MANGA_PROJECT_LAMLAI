...
4. Code cũ trong `src/backend`, `src/frontend`, `docs/database/schema_postgresql_v2.sql` — chỉ dùng để tham khảo workflow/demo behavior, không xem là chuẩn kiến trúc cuối.
...
- `src/backend` và `src/frontend` hiện tại chỉ là baseline/reference.
...
- `api/`: controller/route entry point
- `application/`: service, business rules, workflow/state machine
- `domain/`: entity, DTO, enum, core data shape
- `infrastructure/`: repository, storage, notification, AI connector
- `config/`: DB/server/security configuration

Nếu dùng Java Spring Boot, có thể map tương đương:
...
infrastructure/   -> repository/storage/external clients
